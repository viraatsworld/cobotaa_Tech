"""End-to-end demo launch (variant 2).

Same bringup as `hybrid_planning_demo.launch.py` but starts
`hybrid_planning_demo2_node`, which sends two hard-coded Cartesian goals
(baked into the node) to the Hybrid Planner.

Run with:
    ros2 launch techtory_hybrid_planning hybrid_planning_demo2.launch.py
"""
...
def generate_launch_description():
    ...
    # The two Cartesian goals are hard-coded inside the demo2 node, so the
    # only parameters needed here are the planning context and timeouts.
    demo_client_node = Node(
        package="techtory_hybrid_planning",
        executable="hybrid_planning_demo2_node",
        name="hybrid_planning_demo2_node",
        output="screen",
        parameters=[
