"""End-to-end demo launch (variant 3 — MTC welding global planner).

Same bringup as `hybrid_planning_demo2.launch.py`, but:
  * the Global Planner loads the MoveIt Task Constructor plugin
    (`techtory_hybrid_planning/GlobalMTCPlannerComponent`) via
    `config/global_planner_mtc.yaml`, and
  * the demo client is `hybrid_planning_demo3_node`, which sends a single
    MotionSequenceItem whose goal_constraints[0] carries two position and two
    orientation constraints (start + goal welding poses).
...
def _hybrid_container(context, *args, **kwargs):
    ...
    # Load the MTC global planner instead of the stock MoveItPlanningPipeline.
    return [
        mod.generate_hybrid_planning_container(
            global_planner_config="config/global_planner_mtc.yaml"
        )
    ]
...
def generate_launch_description():
    ...
    # The two welding poses are baked into the demo3 node (override via the
    # weld_start.* / weld_goal.* parameters); only the planning context and
    # timeouts are passed here.
    demo_client_node = Node(
        package="techtory_hybrid_planning",
        executable="hybrid_planning_demo3_node",
        name="hybrid_planning_demo3_node",
        output="screen",
        parameters=[
