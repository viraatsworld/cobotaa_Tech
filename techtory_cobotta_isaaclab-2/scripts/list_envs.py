...
All the environments are registered in the `techtory_cobotta_isaaclab` extension. They start
...
with contextlib.suppress(ImportError):
    import techtory_cobotta_isaaclab.tasks_experimental  # noqa: F401
...
def main():
    """Print all environments registered in `techtory_cobotta_isaaclab` extension."""
    # Collect matching task specs first so we can enumerate presets in one pass.
    task_specs = [
        spec
        for spec in gym.registry.values()
        if "Techtory-Cobotta-" in spec.id and (args_cli.keyword is None or args_cli.keyword in spec.id)
    ]
